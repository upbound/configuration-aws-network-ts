...
/**
 * compose creates AWS network infrastructure including VPC, subnets,
...
export const compose: ComposeFunction = async (req, rsp, logger) => {
  ...
  try {
    ...
    // Desired composed resources, written straight onto the response.
    // ComposeResponse narrows desired to non-optional, so no rsp.desired!.
    const desiredComposed = rsp.desired.resources;
